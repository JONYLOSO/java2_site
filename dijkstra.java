...
class dijkstra{
    ...
    public static void main(String[] args) {
                                  //  0  1  2  3  4  5  6  
      int map[][] = new int[][] { { 0, 2, 1, 4, 0, 0, 0 }, // 0
                                  { 2, 0, 2, 0, 3, 0, 0 }, // 1
                                  { 1, 2, 0, 2, 5, 7, 0 }, // 2
                                  { 4, 0, 2, 0, 0, 4, 0 }, // 3
                                  { 0, 3, 5, 0, 0, 0, 3 }, // 4
    ...
    public static void printTableSolution(int point[], String route[], boolean visited[],int startPoint) {
        System.out.println();
        line();
        System.out.printf("\t\t\tThe shortest distance from the point  %d\n",startPoint);
        line();
        System.out.printf("|    Visited    |    To Point   |     Distance From %2d     |\t  Shortest Route \t|\n",startPoint);
        line();
    ...
    public static int minDistance(int point[], boolean visited[], int startPoint) {
        ...
        line();
        System.out.println(" *** Start at point a then pass point "+startPoint+" and finish at point "+minIndex+" and point not visited ***");
        return minIndex;
    }
    ...
    public static void dijkstra(int map[][], int startPoint) {
        ...
        for (int i = 0; i < map.length; i++) {
            point[i] = 999999;          // Like infinite  (This case range 0-999999);
            visited[i] = false;         
        ...
        for (int i = 0; i < map.length ; i++) {
            ...
            for (int visit = 0; visit < map.length; visit++) {
                if (!visited[visit] && map[start][visit] != 0 && point[start] != 999999) {
                    ...
                    if (point[start] + map[start][visit] < point[visit]) {
                        ...
                        System.out.printf("\t\t- Start => %3d  To => %3d  Distance => %3d  Route => %s\n",startPoint,visit,point[start] + map[start][visit],route[start] + String.valueOf(visit));
                        ...
                        System.out.printf("\t\t- Start => %3d  To => %3d  Distance => %3d  Route => %s\n",startPoint,visit,point[start] + map[start][visit],route[start] + String.valueOf(visit));
                        System.out.printf("\t\t- More\n\t\t- Start => %3d  To => %3d  Distance => %3d  Route => %s\n",startPoint,visit,point[start],route[visit]);
                    }
                    System.out.println("------------------------------------------------------------------------------------------");
                }else{
                    ++checkFind;
            ...
            System.out.print("Next step...(Enter)"); sc.nextLine();
            System.out.println("");;
            printTableSolution(point, route, visited,startPoint);
        }
    }
